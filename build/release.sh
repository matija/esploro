...
if [[ -z "${RELEASE_NOTES:-}" ]]; then
  PREVIOUS_RELEASE_TAG=$(git tag --list --sort=-v:refname \
    | grep -E '^v?[0-9]+\.[0-9]+\.[0-9]+$' \
    | grep -vFx "$RELEASE_TAG" \
    | grep -vFx "v$RELEASE_TAG" \
    | head -1) || PREVIOUS_RELEASE_TAG=""
