...
pub(super) fn build_pg_insert_preview_statement(
    schema: &str,
    table: &str,
    row: &NewRowValues,
    col_type_map: &HashMap<String, String>,
        ...
        let part = match &cc.value {
            ...
            Some(val) => {
                ...
                if udt.starts_with('_') {
                    let elem_type = udt.trim_start_matches('_');
                    let pg_array = json_to_pg_array_literal(val)?;
                    format!("'{}'::{}[]", sql_escape_string(&pg_array), elem_type)
                } else {
                    let cast = col_cast_map
                        .get(&cc.column)
                        .map(|s| s.as_str())
...
pub(super) fn build_pg_insert_preview_sql(
    schema: &str,
    table: &str,
    rows: &[NewRowValues],
    col_type_map: &HashMap<String, String>,
    ...
    let mut statements: Vec<String> = vec![];
    for row in rows {
        statements.push(build_pg_insert_preview_statement(
            schema,
            table,
            row,
            col_type_map,
...
pub(super) fn build_mysql_insert_preview_statement(
    schema: &str,
    table: &str,
    row: &NewRowValues,
) -> Result<String, String> {
    ...
    if row.column_values.is_empty() {
        return Ok(format!("INSERT INTO `{schema}`.`{table}` VALUES ();"));
    }
    ...
    for cc in &row.column_values {
        columns.push(format!("`{}`", cc.column));
        value_parts.push(match &cc.value {
            Some(val) => format!("'{}'", sql_escape_string(val)),
            None => "NULL".to_string(),
        });
    }
...
pub(super) fn build_mysql_insert_preview_sql(
    schema: &str,
    table: &str,
    rows: &[NewRowValues],
) -> Result<String, String> {
    let mut statements: Vec<String> = vec![];
    for row in rows {
        statements.push(build_mysql_insert_preview_statement(schema, table, row)?);
    }
...
pub(super) fn build_pg_insert_sql(
    ...
    if row.column_values.is_empty() {
        return Err("Insert row has no column values".to_string());
    }
    for cv in &row.column_values {
        validate_column_identifier(&cv.column)?;
    }
    ...
    for cv in &row.column_values {
        columns.push(format!("\"{}\"", cv.column));
        if let Some(ref val) = cv.value {
            let udt = col_type_map
                .get(&cv.column)
                .map(|s| s.as_str())
                .unwrap_or("text");
...
mod tests {
    ...
    fn new_row(values: Vec<ColumnChange>) -> NewRowValues {
        NewRowValues {
    ...
    fn pg_insert_sql_uses_casted_params_for_each_column() {
        ...
        let sql = build_pg_insert_sql(
            ...
            &row,
            &map(&[("name", "text")]),
            &map(&[("name", "text"), ("nickname", "text")]),
        )
        .unwrap();

        assert_eq!(
            sql.sql,
            r#"INSERT INTO "public"."people" ("name", "nickname") VALUES ($1::text::text, NULL)"#
        );
        assert_eq!(sql.params, vec!["O'Hara".to_string()]);
    }
    ...
    #[test]
    fn pg_insert_sql_errors_without_columns() {
        let row = new_row(vec![]);

        assert_eq!(
            build_pg_insert_sql("public", "people", &row, &HashMap::new(), &HashMap::new())
                .unwrap_err(),
            "Insert row has no column values"
        );
    }
    ...
    #[test]
    fn mysql_insert_sql_binds_placeholders_and_keeps_nulls_inline() {
        let row = new_row(vec![
            set("name", Some("O'Hara")),
            set("nickname", None),
        ]);
    ...
    #[test]
    fn mysql_insert_sql_errors_without_columns() {
        let row = new_row(vec![]);

        assert_eq!(
            build_mysql_insert_sql("app", "people", &row).unwrap_err(),
            "Insert row has no column values"
        );
    }
