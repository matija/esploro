//! MySQL leg of the `integration-db` suite. Skips cleanly when
//! `ESPLORO_TEST_MYSQL_URL` isn't set.
...
const ENV_VAR: &str = "ESPLORO_TEST_MYSQL_URL";
...
async fn schema_introspection_and_row_mutations_round_trip() {
    ...
    let harness = common::setup_mysql(&url).await;
    let state = harness.state();
    let session_id = harness.session_id.clone();
    // For MySQL, `schema` in every command below is the database name — there
    // is no separate schema level (see commands/schema.rs).
    let database = common::db_name_from_url(&url);
    let table = common::unique_table_name("esploro_it_mysql");

    let create = data::execute_sql(
        session_id.clone(),
        format!(
            "CREATE TABLE `{table}` (id int primary key auto_increment, name varchar(255) not null)"
        ),
        state.clone(),
    )
    .await
    ...
    let insert = data::execute_sql(
        session_id.clone(),
        format!("INSERT INTO `{table}` (name) VALUES ('alpha'), ('beta')"),
        state.clone(),
    )
    .await
